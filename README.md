...
一个支持登录注册、文档管理（无限级目录树 / 全文搜索 / 标签筛选）、标签管理的全栈项目。
...
> 表结构由 MyBatis-Plus 启动时自动创建（按需），也可手动执行 `backend/src/main/resources/schema.sql`（如有）。
...
- 后端 Spring Boot 代码由作者**独立手写完成**，包括：数据库设计、接口开发、JWT 认证、权限控制、目录树、标签系统、全文搜索。
- 前后端联调由作者独立完成。
